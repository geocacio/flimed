...
module.exports = app => {
    ...
    const viewDoctorResultado = async (req, res) => {
        await User.findOne({ _id: req.session.user._id }).then(user => {
            res.status(200).render('medico-resultado',{
                page: 'Resultado',
                user,
                message: null
            })
    ...
    const viewDoctorPesquisaAvancada = async (req, res) => {
        await User.findOne({ _id: req.session.user._id }).then(user => {
            res.status(200).render('medico-pesquisa-avancada',{
    ...
    const viewDoctorMarcarConsulta = async (req, res) => {
        await User.findOne({ _id: req.session.user._id }).then(user => {
            res.status(200).render('medico-marcar-consulta',{
    ...
    const viewDoctorDetalhes = async (req, res) => {
        await User.findOne({ _id: req.session.user._id }).then(user => {
            res.status(200).render('medico-detalhes',{
                page: 'Marcar Consulta',
                user,
                message: null
            })
    ...
    return { 
        ...
        viewDoctorQuestions,
        viewDoctorResultado,
        viewDoctorPesquisaAvancada,
        viewDoctorMarcarConsulta,
        viewDoctorDetalhes,
        viewDoctorAdverts,
